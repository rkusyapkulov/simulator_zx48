...
extern unsigned char ReadByte(unsigned short addr);
extern void WriteByte(unsigned short addr, unsigned char val);
extern unsigned char InPort(unsigned short port);
extern void OutPort(unsigned short port, unsigned char val);


#define Z80_READ_BYTE(address, x)                                       \
{                                                                       \
        (x) = ReadByte((address) & 0xffff);                             \
}
...
{                                                                       \
        WriteByte((address) & 0xffff, (x));                              \
}

#define Z80_WRITE_WORD(address, x)                                      \
{                                                                       \
        WriteByte((address) & 0xffff, (x & 0xff));                       \
        WriteByte(((address) + 1) & 0xffff, (x >> 8));                  \
}
...
{                                                                       \
        /* Восстанавливаем оригинальный 16-битный порт из регистров */  \
        u16 real_port = (state->registers.byte[Z80_C] | (state->registers.byte[Z80_B] << 8)); \
        /* Если это была инструкция IN A, (n), то адрес берется из аккумулятора и шины */ \
        if (opcode == 0xDB) {                                           \
            real_port = ((port & 0xFF) | (state->registers.byte[Z80_A] << 8)); \
        }                                                               \
        (x) = InPort(real_port);                                        \
}
