...
// --- СОВМЕСТИМОСТЬ С СИ-СТРУКТУРОЙ ИЗ Z80.C ---
typedef uint8_t  u8;
...
struct z80_cpp : public z80 {
    ...
    inline void step() { ::z80_step(this); }
...
// Helper macros/functions updated to route through cpu.F and the new registers
static inline u16 GetHL() { return ((u16)cpu.h << 8) | cpu.l; }
static inline void SetHL(u16 val) { cpu.h = (u8)(val >> 8); cpu.l = (u8)(val & 0xFF); }
static inline u16 GetBC() { return ((u16)cpu.b << 8) | cpu.c; }
static inline void SetBC(u16 val) { cpu.b = (u8)(val >> 8); cpu.c = (u8)(val & 0xFF); }
static inline u16 GetDE() { return ((u16)cpu.d << 8) | cpu.e; }
static inline void SetDE(u16 val) { cpu.d = (u8)(val >> 8); cpu.e = (u8)(val & 0xFF); }

static inline u8 pack_flags() {
    return (cpu.cf ? 0x01 : 0x00) |
        (cpu.nf ? 0x02 : 0x00) |
        (cpu.pf ? 0x04 : 0x00) |
        (cpu.xf ? 0x08 : 0x00) |
        (cpu.hf ? 0x10 : 0x00) |
...
static inline void unpack_flags(u8 val) {
    cpu.cf = (val >> 0) & 1;
    cpu.nf = (val >> 1) & 1;
    cpu.pf = (val >> 2) & 1;
    cpu.xf = (val >> 3) & 1;
...
// Заменяем старые get_f / set_f
static inline u8 get_f() { return pack_flags(); }
static inline void set_f(u8 val) { unpack_flags(val); }
...
extern "C" u8 ReadByte(void*, u16 addr)
{
    WORD target = addr;
...
extern "C" void WriteByte(void*, u16 addr, u8 val)
{
    WORD target = addr;
...
extern "C" u8 InPort(z80* const z, u8 port_low) {
    // Восстанавливаем полный 16-битный порт, беря старший байт из регистра B (или А для IN A,(n))
    u16 port = ((u16)cpu.b << 8) | port_low;
...
extern "C" void OutPort(z80* const z, u8 port_low, u8 val) {
    // Восстанавливаем полный 16-битный порт, беря старший байт из регистра B (или А для IN A,(n))
    u16 port = ((u16)cpu.b << 8) | port_low;
...
static inline void WriteByteInline(int addr, BYTE val) { WriteByte(NULL, (u16)addr, (u8)val); }
static inline BYTE ReadByteInline(int addr) { return (BYTE)ReadByte(NULL, (u16)addr); }
    ...
    if (cpu.pc == 0x0556 && tap_is_loaded) {
        if (tap_current_pos + 2 <= current_tap_data.size()) {
            ...
            if (tap_current_pos + 2 + block_len <= current_tap_data.size()) {
                ...
                WORD target_addr = cpu.ix; // Capitalized IX
                for (WORD i = 0; i < data_len; i++) {
                    WriteByte(NULL, target_addr + i, raw_data[i]);
                }

                tap_current_pos += 2 + block_len;

                cpu.cf = 1; // Успешный Carry
                cpu.zf = 1; // Успешный Zero
                cpu.a = 0u; // Capitalized A

                WORD low = ReadByte(NULL, cpu.sp);  cpu.sp = (cpu.sp + 1) & 0xFFFF;
                WORD high = ReadByte(NULL, cpu.sp); cpu.sp = (cpu.sp + 1) & 0xFFFF;
                cpu.pc = (high << 8) | low;
...
int SystemStepZ80()
{
    ...
    // --------------------------------------------------------
    unsigned long old_cyc = cpu.cyc;
    z80_step(&cpu);
    return (int)(cpu.cyc - old_cyc);
}
...
static bool LoadSnaSnapshot(const std::vector<BYTE>& data, std::wstring& error) {
    ...
    cpu.i = h[0];
    cpu.l_ = h[1]; cpu.h_ = h[2];
    cpu.e_ = h[3]; cpu.d_ = h[4];
    cpu.c_ = h[5]; cpu.b_ = h[6];
    cpu.f_ = h[7]; cpu.a_ = h[8];
    ...
    cpu.interrupt_mode = (BYTE)(h[25] & 0x03);
    if (cpu.interrupt_mode > 2) cpu.interrupt_mode = 2;
...
static bool LoadZ80Snapshot(const std::vector<BYTE>& data, std::wstring& error) {
    ...
    SetDE(ReadLE16(h + 13));
    cpu.c_ = h[15]; cpu.b_ = h[16];
    cpu.e_ = h[17]; cpu.d_ = h[18];
    cpu.l_ = h[19]; cpu.h_ = h[20];
    cpu.a_ = h[21]; cpu.f_ = h[22];
    cpu.iy = ReadLE16(h + 23);
    cpu.ix = ReadLE16(h + 25);
    cpu.iff1 = h[27] != 0;
    cpu.iff2 = h[28] != 0;
    cpu.interrupt_mode = (BYTE)(h[29] & 0x03);
    if (cpu.interrupt_mode > 2) cpu.interrupt_mode = 2;
...
void UpdateRegisterDisplay() {
    ...
    swprintf(buf, 64, L" REG A: %02Xh F: %02Xh", cpu.a, get_f());
    SendMessageW(hRegListBox, LB_ADDSTRING, 0, (LPARAM)buf);
    swprintf(buf, 64, L" REG BC: %04Xh", GetBC());
    SendMessageW(hRegListBox, LB_ADDSTRING, 0, (LPARAM)buf);
    swprintf(buf, 64, L" REG DE: %04Xh", GetDE());
    SendMessageW(hRegListBox, LB_ADDSTRING, 0, (LPARAM)buf);
    swprintf(buf, 64, L" REG HL: %04Xh", GetHL());
    SendMessageW(hRegListBox, LB_ADDSTRING, 0, (LPARAM)buf);
