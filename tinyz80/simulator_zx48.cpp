...
extern "C" {
    // 1. Объявляем плоские Си-функции, которые реально существуют в z80.c
    void init(void);
    int step(void);
    void gen_nmi(void);
    ...
    // 2. Объявляем структуру в точности такой, какая она в z80.c
    struct CPUZ80 {
        u16 PC, SP, IX, IY;
        u16 wz;
        u8 A, B, C, D, E, H, L;
        ...
        // Вместо __thiscall методов делаем inline-вызовы плоских Си-функций.
        // Это сохраняет синтаксис cpu.step() и cpu.init() во всём C++ коде!
        inline void init() { ::init(); }
        inline int step() { return ::step(); }
        inline void gen_nmi() { ::gen_nmi(); }
        inline void gen_int(u8 data) { ::gen_int(data); }
        inline int z80_interrupt_now(u8 data) { return ::z80_interrupt_now(data); }
    };

    // Глобальный объект процессора, выделенный в z80.c
    extern struct CPUZ80 cpu;
}
...
static inline u8 get_f() { return cpu.F; }
static inline void set_f(u8 val) { cpu.F = val; }
    ...
    if (cpu.PC == 0x0556 && tap_is_loaded) {
        if (tap_current_pos + 2 <= current_tap_data.size()) {
            ...
            if (tap_current_pos + 2 + block_len <= current_tap_data.size()) {
                ...
                cpu.F |= (0x01 | 0x40); // Direct bit manipulation on cpu.F
                cpu.A = 0u;             // Capitalized A

                WORD low = ReadByte(cpu.SP);  cpu.SP = (cpu.SP + 1) & 0xFFFF;
                WORD high = ReadByte(cpu.SP); cpu.SP = (cpu.SP + 1) & 0xFFFF;
                cpu.PC = (high << 8) | low;
...
static bool LoadZ80Snapshot(const std::vector<BYTE>& data, std::wstring& error) {
    ...
    cpu.SP = ReadLE16(h + 8);
    cpu.I = h[10];
    cpu.RR = (BYTE)((h[11] & 0x7F) | ((h[12] & 0x01) << 7));
        ...
        if (elapsed > 0.0) {
            ...
            while (internal_debt > 0.0) {
                ...
                if (cycles_until_interrupt <= 0) {
                    ...
                    int int_ticks = z80_interrupt_now(0xFF);
